...
## Functions 
# This function is used to split my dataset in 3 parts
...
def datetime_to_index(dataset, date):
  date_0 = datetime.strptime(dataset[0][0], '%Y-%m-%d') #First date of my dataset (day 0)
  return abs(date - date_0).days

# Turn an array of datetime types inrto an array of indexes
def array_date_to_array_index(dataset, array_index, array_datetime):
  for i in range(np.shape(array_datetime)[0]):
    array_index = np.append(array_index, datetime_to_index(dataset, array_datetime[i][0]))

  array_index = array_index[:, np.newaxis]
  return array_index
...
data_board = [(first_x, first_x_test, first_y1, first_y1_test, first_y2, first_y2_test, 'April 2020 to December 2020', 'yellow', 9, 8, Ridge(alpha=10.0), Ridge(alpha=10.0)),
              (second_x, second_x_test, second_y1, second_y1_test, second_y2, second_y2_test, 'January 2021 to November 2021', 'orange', 10, 4, Ridge(alpha=10.0), LinearRegression()),
              (third_x, third_x_test, third_y1, third_y1_test, third_y2, third_y2_test, 'December 2021 to April 2022', 'red', 7, 4, Ridge(alpha=10.0), LinearRegression())
             ]

## Start the regression system
for this_X, this_X_test, this_y1, this_y1_test, this_y2, this_y2_test, name, color, polynome_number_1, polynome_number_2, model_1, model_2 in data_board:
  indexes_x = np.array([])
  indexes_x_test = np.array([])
  indexes_x = array_date_to_array_index(dataset, indexes_x, this_X)
  indexes_x_test = array_date_to_array_index(dataset, indexes_x_test, this_X_test)

  x_plot = np.linspace(indexes_x.min(), indexes_x.max())
  
  #Daily new cases
  plt.figure(figsize=(10, 10))
  plt.plot(indexes_x[:, 0], this_y1, "b+")

  model = make_pipeline(PolynomialFeatures(polynome_number_1), model_1)
  model.fit(indexes_x, this_y1)
  mse = mean_squared_error(model.predict(indexes_x_test), this_y1_test)/10000000
  y1_plot = model.predict(x_plot[:, np.newaxis])
  plt.plot(x_plot, y1_plot, color=color, linestyle='-', linewidth=3, label="error = %.3f" % mse)
  plt.title('COVID-19 Daily new cases evolution ' + name)

  legend = plt.legend(loc="upper right", frameon=False, prop=dict(size="small"))
  plt.show()

  #Daily deaths
  plt.figure(figsize=(10, 10))
  plt.plot(indexes_x[:, 0], this_y2, "b+")

  model = make_pipeline(PolynomialFeatures(polynome_number_2), model_2)
  model.fit(indexes_x, this_y2)
  mse = mean_squared_error(model.predict(indexes_x_test), this_y2_test)/1000
  y2_plot = model.predict(x_plot[:, np.newaxis])
  plt.plot(x_plot, y2_plot, color=color, linestyle='-', linewidth=3, label="error = %.3f" % mse)
  plt.title('COVID-19 Daily deaths evolution ' + name)
