...
second_y = np.array([]) 
second_y_test = np.array([])
third_x = np.array([]) 
third_x_test = np.array([]) 
third_y = np.array([]) 
third_y_test = np.array([])


first_x, first_x_test, first_y, first_y_test = split_data(dataset, first_x, first_x_test, first_y, first_y_test, 0, 150)
second_x, second_x_test, second_y, second_y_test = split_data(dataset, second_x, second_x_test, second_y, second_y_test, 150, 280)
third_x, third_x_test, third_y, third_y_test = split_data(dataset, third_x, third_x_test, third_y, third_y_test, 280, np.shape(dataset)[0])
...
data_board = [
    (first_x, first_x_test, first_y, first_y_test, "April 2020 to December 2020", 8, 2000),
    (second_x, second_x_test, second_y, second_y_test, "January 2021 to November 2021", 4, 1500),
    (third_x, third_x_test, third_y, third_y_test, "December 2021 to April 2022", 4, 1500)
]
...
linestyle = {"OLS": "-", "Theil-Sen": "--", "RANSAC": "-", "Lasso": "--", "Ridge": "-"}
lw = 3

for this_X, this_X_test, this_y, this_y_test, period, p, y_lim in data_board:
    indexes_X = np.array([])
    ...
    plt.plot(indexes_X[:, 0], this_y, "b+")

    for name, estimator in estimators:
        model = make_pipeline(PolynomialFeatures(p), estimator)
        model.fit(indexes_X, this_y)
        mse = mean_squared_error(model.predict(indexes_X_test), this_y_test)
        ...
        plt.plot(
            ...
            linewidth=lw,
            label="%s: error = %.3f" % (name, mse/1000),
        )
        plt.scatter(indexes_X_test, this_y_test)

    legend = plt.legend(loc="upper right", frameon=False, title="", prop=dict(size="small"))
    plt.title("COVID-19 Daily new cases evolution during " + period)
    plt.ylim(-100, y_lim)
plt.show()